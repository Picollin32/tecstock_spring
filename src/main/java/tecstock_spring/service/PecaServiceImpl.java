...
public class PecaServiceImpl implements PecaService {
    ...
    @Override
    public List<tecstock_spring.dto.PecaPesquisaDTO> listarUltimosParaInicio(int limit) {
        Long empresaId = TenantContext.getCurrentEmpresaId();
        if (empresaId == null) {
            throw new IllegalStateException("Empresa não encontrada no contexto do usuário");
        }
        ...
        return pecaRepository.findTopByEmpresaIdOrderByCreatedAtDesc(empresaId, pageable);
    }
}